...
// GETTERS
::GetEffectFlags <- function(any_ent) 
{   // Returns the value of m_fEffects. SEE "ENTITY EFFECTS" in const.nut
    return NetProps.GetPropInt(any_ent, "m_fEffects");
}

// SETTERS
::AddEffectFlag <- function(any_ent, flag)
{   // Adds a effect flag to the entity. SEE "ENTITY EFFECTS" in const.nut
    return NetProps.SetPropInt(any_ent, "m_fEffects", (::GetEffectFlags(any_ent) | flag));
}

::RemoveEffectFlag <- function(any_ent, flag)
{   // Removes a effect flag from the entity. SEE "ENTITY EFFECTS" in const.nut
    return NetProps.SetPropInt(any_ent, "m_fEffects", (::GetEffectFlags(any_ent) & ~flag));
}
...
::IsInTrigger <- function(origin, trigger_ent)
{   // True is the ent is in trigger. It's the closest solution we have from L4D2's IsTouching()
    trigger_ent.RemoveSolidFlags(FSOLID_NOT_SOLID)  // FSOLID_NOT_SOLID
	local trace =
	{
		start = origin
		end   = origin
	...
	TraceLineEx(trace);
	trigger_ent.AddSolidFlags(FSOLID_NOT_SOLID);

	return trace.hit && trace.enthit == trigger_ent;
}